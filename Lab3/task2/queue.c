...
#define CONFIG_FILE "deeds_fifo_stats"
...
static int in, out = 0;
static int queue_size = 32, counter=1, numberOfItems = 0;
static int total_insert = 0, total_removal = 0;
 ...
 */
static int stats_read_Changed(struct seq_file *m, void *v) {
    printk(KERN_INFO
    "Printing stats\n");

    int percentage = (int)((numberOfItems  * 100)/ queue_size); 

    seq_printf(m, "Number of Stored Items: %d\nNumber of Empty Slots: %d\nPercentage of Filled Slots: %d\nCurrent DataItem SeqNumber: %d\n",
        numberOfItems, (queue_size - numberOfItems), percentage, counter);
    
    seq_printf(m, "Total Insertions: %d\nTotal Removals: %d\n", total_insert, total_removal);
...
static int stats_open(struct inode *inode, struct file *file) {
    return single_open(file, stats_read_Changed, NULL);
}
...
fifo_module_init(void) {
    ...
    proc_entry_ptr_config = proc_create(CONFIG_FILE, FILE_ACCESS_RONLY, NULL, &file_ops_config);
...
module_param(queue_size, int, S_IWUSR);
